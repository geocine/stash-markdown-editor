...
(function(){
  ...
  showdown.Converter = function (converterOptions) {
    ...
    this.makeHtml = function (text) {
      ...
      // if there is no space between BR and ``` treat it as codeSpan
      const brPattern = /(<br\s*\/>.*[\r\n]{1}\`\`\`)/g;
      let match = [];
      while (match = brPattern.exec(text)) {
        const start = match.index + match[0].length;
        const end = text.indexOf('```',match.index + match[0].length);
        text = replaceAt(
          text,text.substring(match.index, end + 3),
          '<br />`' + text.substring(start, end) + '`', 
          match.index, end + 3);
      }
